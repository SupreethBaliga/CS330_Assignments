...
int main(u64 arg1, u64 arg2, u64 arg3, u64 arg4, u64 arg5)
{
	...
	if(cpid < 0){
		...
		// wait for breakpoint fn
		ret = wait_and_continue();
		printf("wait_and_continue() for fn =  %x\n", ret);
		...
		// wait for fn_1		
		ret = wait_and_continue();
		printf("wait_and_continue() for fn_1= %x\n", ret);
        info_registers(&reg_info);	
		printf("Registers:\n");
		printf("r15: %x\n", reg_info.r15);
		printf("r14: %x\n", reg_info.r14);
        ...
        //wait for fn_2
        ret = wait_and_continue();
        printf("wait_and_continue() for fn_2= %x\n", ret);
        info_registers(&reg_info);	
		printf("Registers:\n");
		printf("r15: %x\n", reg_info.r15);
		printf("r14: %x\n", reg_info.r14);
        ...
        //waiting for exit
        ret = wait_and_continue();
        printf("wait_and_continue() for = %x\n", ret);
	}
