...
int sys_become_debugger(struct exec_context *ctx)
{
	return do_become_debugger(ctx);
}

int sys_set_breakpoint(struct exec_context *ctx, u64 addr)	
{
	return do_set_breakpoint(ctx, (void *)addr);
}
...
int sys_enable_breakpoint(struct exec_context *ctx, u64 addr)
{
	return do_enable_breakpoint(ctx, (void *)addr);
}

int sys_disable_breakpoint(struct exec_context *ctx, u64 addr)
{
	return do_disable_breakpoint(ctx, (void *)addr);	
}
...
long  do_syscall(int syscall, u64 param1, u64 param2, u64 param3, u64 param4)
{
	...
	case SYSCALL_BECOME_DEBUGGER:
		return sys_become_debugger(current);	
	case SYSCALL_SET_BREAKPOINT:
		return sys_set_breakpoint(current, param1);	
	case SYSCALL_REMOVE_BREAKPOINT:
		return sys_remove_breakpoint(current, param1);
	case SYSCALL_ENABLE_BREAKPOINT:
		return sys_enable_breakpoint(current, param1);
	case SYSCALL_DISABLE_BREAKPOINT:
		return sys_disable_breakpoint(current, param1);
